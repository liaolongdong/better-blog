...
#
# 同一族的第二个错：数「元素」之前不挖脚本。`grep -o '<img'` 会把 <script> 里
# 当作字符串字面量写的 `<img class="wl-emoji">` 一起数进来（waline 的评论组件就有），
# 于是一张根本不存在的图被当成"缺 alt / 缺懒加载"的缺陷上报。本仓库的分析报告里
# "1355 个 <img>、264 个缺 alt"三个数就是这么来的，复核后作废（真实是 1036 个、
