...
| 项 | 预算 / 规则 | 超了怎么办 |
|---|---|---|
| 区划表产物 | gzip ≤ **34KB**，且**只挂证件页** | 再压：区划只存 `code→[省index,市index,名]` 三级索引 |
...
30KB 那条是估的，实测要收：县级表分组编码（`1101,01东城区 02西城区…`）后四张表合计 92.6KB 原始 / **31.9KB gzip**（历史表不分组时为 32.5KB）；三级索引那档压法留给真超预算时再用，现在多出来的 4KB 换来的是"生成器与解析器各只有 10 行、出错一眼看得懂"。断言写成 gzip ≤ 34KB，留出刷新数据后条数增长的余量。
