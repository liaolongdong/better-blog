...
预算取 36KB 而不是贴着实测的 34KB：同一份字节在 `level 6/9` 之间实测相差 194B，换 `strategy` 到 `Z_FILTERED` 在默认 level 下高 2,275B（level 9 下高 2,036B）——34KB 只剩 9 字节余量，会被压缩器版本与默认参数的抖动直接抖翻，那条判据就从"防体积回退"变成了"防今天用哪个 Node"。**抬预算吸收的是抖动，不是数据增长**：同样这些信息量（四张表 31/342/2,978/1,229 行，列名 `provinces{code,name}` / `cities{code,provinceCode,name}` / `counties{code,cityCode,name}` / `historical{code,name}`）退回朴素 `JSON.stringify` 实测 260,533B 原文 / 47,443B gzip（level 6）/ 44,425B（level 9），三种口径都在 36,864B 之上；`--fetch` 刷新快照带来的条数增长同样该红，红了要人重新决定预算，这是设计意图不是误报。
