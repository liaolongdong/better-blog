...
**统一社会信用代码（GB 32100-2015）**　字符集 31 个：`0-9A-Z` 去掉 `I O S Z V`，值按序 0..30；`Wi = 3^(i-1) mod 31` = `1,3,9,27,19,26,16,17,20,29,25,13,8,24,10,30,28`；`C18 = 31 − (Σ Vi·Wi mod 31)`，为 31 时取 `0`。
...
- 站内现有的 `GB2260.js`（npm `id-validator@1.3.0`，MIT，作者 mc-zone）共 **3,465 条**，实测仍含 `110103 北京市崇文区`（2010 撤销）、`110104 宣武区`（2010 撤销）、`110221 昌平县`（2015 改区）、`110228 密云县`（2015 改区）、`371200 山东省莱芜市`（2019 并入济南），缺 `653224 和田地区和安县`（2020 设）。
...
| `#uscc` | 统一社会信用代码 | **生成**：登记管理部门（11 类）、机构类别、行政区划（现行码，市级起）→ 合规号码，第 9–17 位用真实结构（8 位本体 + `GB/T 11714` 校验位）。**校验/解析**：三态结论 + 逐段解析（登记部门名、机构类别名、区划名、主体标识、内层组织机构代码校验位、USCC 校验位算式）；显式提示 `I O S Z V` 不属字符集，出现即判结构非法 |
...
- 区划回落链：6 位县级 → 4 位市级 → 2 位省级，每一级命中都要在结果里说明回落到哪一级，绝不出现 §2.2 那种「未知地区」。历史码命中要标「已撤销建制（现行码：xxx，若有）」。
...
dev/js/tools/region-data.js    生成产物：现行 + 历史两张区划表
...
| 项 | 预算 / 规则 | 超了怎么办 |
|---|---|---|
| 区划表产物 | gzip ≤ 30KB，且**只挂证件页** | 再压：区划只存 `code→[省index,市index,名]` 三级索引 |
...
- 区划表：无 6 位重复、现行 ∩ 历史 = ∅、现行计数 == 源 JSON 计数；抽查 `110101`（现行·东城区）、`653224`（现行·和安县）、`110103`（历史且带撤销标记）、一个仅市级可回落的码。
...
| 真实企业码样本不足导致 USCC 判据是"自证" | 实现前先把 ≥5 条可公开核实的真实码连同来源写进测试文件注释；不足就把该项标为未验证，不假装过 |
