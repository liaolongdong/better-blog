...
- `11010119900307355` → 算得 `4`；站内旧库判 `110101199003073554` 有效、`…558` 无效 → **两个独立实现同结论**。（顺带纠正一个流传很广的"示例号"记忆：末位是 4 不是 8。）
...
- 站内现有的 `GB2260.js`（npm 包 `id-validator`，MIT，作者 mc-zone；**版本以仓库内的文件头为准 = `IDValidator.js` 标的 v1.2.0**，本节初稿写的"1.3.0"是照着 npm 页面抄来的，站内没有可核证据）共 **3,465 条**（2026-09-25 复算：`Object.keys(require('./demo/idCardDemo/lib/GB2260.js')).length` = 3,465，与冻结快照 `scripts/fixtures/region-source/gb2260-2015.json` 逐字节同集），实测仍含 `110103 北京市崇文区`（2010 撤销）、`110104 宣武区`（2010 撤销）、`110221 昌平县`（2015 改区）、`110228 密云县`（2015 改区）、`371200 山东省莱芜市`（2019 并入济南）——这五条 2026-09-25 逐条从该文件读出，值与左边一致。
...
| 候选 | 实测 | 决定 |
|---|---|---|
| `id-validator`（仓库内 v1.2.0） | MIT / 0 依赖 / 码表 2015 年前后 | 只当**对拍参照物**，不进包 |
...
- 身份证：§2.2 已验算的样本固化为用例；15/18 位、闰年 2 月 29、非闰 2 月 29、12 月 31、校验位 `X`、末位大小写 `x`、含空格、超长、非数字；**与站内旧库对拍**——"两个独立实现同结论才算过"这条标准不变，落地形状在 2026-09-25 第一段实现时改过一次：本行原写"对拍 10 万条随机号"，那样的对拍跑完即失、仓库里不留证据，段 5 删掉 `demo/idCardDemo/` 之后更没有任何东西可以继续比。现在是**固化 1,000 条**进 `scripts/fixtures/id-validator-checkbit-1000.json`（248,934B，`sha256 16971baa…`），七组配额 670/100/100/50/30/30/20 由判据侧写死的常量对账（B7），每条样本的旧库结论由生成器在写盘前**实时跑旧库**得到并原样入文件（旧库一行未改），生成器自身幂等由 B14 复算。取样多样性按"组内去重区划码 ≥ 组内条数一半"钉住（实读 557/97/94/41/30/30/20）。
- USCC：标准示例 + 从企业公示信息里取 ≥5 条真实码；断言字符集恰为 31 个且不含 `IOSZV`；断言 `Wi === 3^(i-1) mod 31` 逐位成立；断言内层组织机构代码校验位与 USCC 校验位在同一真实码上自洽。**本行"≥5 条"没有兑现**，落地形状按 §11 风险表里那一行记录的实测收口：2026-09-25 依次试取 4 个公开来源，只坐实国标示例 `91350100M000100Y43` 这一条可核验样本（凭记忆写的 4 条候选有 3 条校验位不通），所以判据是"1 条真实码 + 生成器自造的码"，并且 `C8 生成侧自洽` 里那句 `assert.equal(REAL_SAMPLES.length, 1, …)` 把这个数目钉死——将来真取到更多样本，必须先让这条判据变红、再回来改本行，"≥5 条已过"这种读法在仓库里不存在。其余三条断言照原样成立，分别由 `C1`（31 字符集、剔除 `IOSZV`）、`C2`（两套权重与幂算式互验）、`C3`（同一真实码上内外两层自洽）钉着。
