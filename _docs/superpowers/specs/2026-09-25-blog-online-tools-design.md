...
体积以落地实测为准（2026-09-25，输入钉在 pin `6fb5380d`）：产物 `dev/js/tools/region-data.js` 为 100,020 字节 / **34,807B gzip = 33.99KB**。写设计时的两个估计（分组编码 92.6KB/31.9KB、历史表不分组 32.5KB）都偏低约 2KB，差额未逐项归因，只把预算按实测算钉。三级索引那档压法留给真超预算时再用，现在多出来的量换来的是编解码各只有一层 `split` 与一层循环——实测落地后生成器 374 行、读侧 369 行，其中编码本身占不到两成，其余是输入闸门与回落链。

预算取 36KB 而不是贴着实测的 34KB：同一份字节在 `level 6/9` 之间实测相差 194B，换 `strategy` 到 `Z_FILTERED` 在默认 level 下高 2,275B（level 9 下高 2,036B）——34KB 只剩 9 字节余量，会被压缩器版本与默认参数的抖动直接抖翻，那条判据就从"防体积回退"变成了"防今天用哪个 Node"。**抬预算吸收的是抖动，不是数据增长**：同样这些信息量退回朴素 `JSON.stringify` 实测三版反解口径都在预算之上——`{code,name}` / `{code,provinceCode,name}` / `{code,cityCode,name}` / `{code,name}` 版 260,533B 原文 / 47,443B gzip（level 6）/ 44,425B（level 9），复核另一版 262,233B / 47,559B / 44,578B，四份快照原样（历史层是 3,465 条全量）451,217B / 66,821B / 62,819B。但这句得连着构造说：**只去掉历史层剩三表是 189,401B / 34,475B / 32,309B，朴素编码照样过得了 36,864B 的预算**——超的量出在那 1,229 条历史码上（朴素口径多 12,968B gzip），所以判据咬的是"编码退回朴素**且**数据仍带历史层"这一整件事。`--fetch` 刷新快照带来的条数增长同样该红，红了要人重新决定预算，这是设计意图不是误报。
