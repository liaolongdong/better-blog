...
- **第四轮规格复核三处，都是"数字抄错了却写成实测"**：
  ...
  ③ **§7 的"读侧 369 行"是被上一轮自己改红的**：369 是 `3723917` 的值，`6573e23` 补完 A5 三条断言之后是 372 行（逐 commit 核过：`eed1b52`=347、`3723917`=369、`6573e23`=372）。同类漂移这次一并防住：计划里三块代码的行号区间全部由脚本按内容锚点重算，本轮收口为判据 254–1023 / 生成器 1057–1430 / 解析器 1466–1837，与磁盘逐字节一致。
