...
(function ($) {
  ...
  var colors = ['#96C2F1', '#BBE1F1', '#E3E197', '#F8B3D0', '#FFCC00']; // 初始许愿墙
  ...
  var createItem = function createItem(card) {
    var color = colors[parseInt(Math.random() * 5, 10)];
    $('<div class="item"><p>' + card.text + '</p><a id="' + card.id + '" href="#">删除</a></div>').css({
      'background': color
  ...
  var init = function init() {
    container = $('#container'); // 绑定关闭事件

    container.on('click', 'a', function () {
      var _this = this;

      // 删除对应的卡片
      wishCards = wishCards.filter(function (item) {
        return item.id !== _this.id;
      }); // 存储到本地localStorage

      localStorage.setItem('wishCards', JSON.stringify(wishCards));
      $(this).parent().remove();
    }).height($(window).height() - 200 < 0 ? 520 : $(window).height() - 200).width($(window).width() - 200 < 0 ? '100%' : $(window).width()); // 渲染许愿墙卡片
    ...
    $('#submit').click(function (e) {
      ...
      if (value) {
        ...
        createItem(cardObj); // 添加到卡片列表

        wishCards.push(cardObj); // 存储到本地localStorage
