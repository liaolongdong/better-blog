...
(function () {
  // 折线图特殊节日配置
  var graphicOption = [];
  var pcGraphicOption = [// graphic 是原生图形元素组件
  {
    type: 'group',
  ...
  var h5GraphicOption = [// graphic 是原生图形元素组件
  {
    type: 'group',
    // 用 setOption 首次设定图形元素时必须指定
    bounding: 'all',
  ...
  var yearReportUser = {
    ...
    init: function init() {
      // 初始化折线图
      this.lineChartInit(); // 初始化饼状图

      this.pieChartInit(); // 初始化环状图

      this.ringlikeChartInit(2853, 262, 9.18);
    },
    // 初始化折线图
    lineChartInit: function lineChartInit() {
      // 初始化echarts实例
      var lineChart = echarts.init(document.getElementById('lineChart')); // 图表配置参数
      ...
      var option = {
        ...
        series: [{
          ...
          type: 'line',
          // 折线图
          silent: Boolean(device),
          // 图形是否不响应和触发鼠标事件，默认为 false，即响应和触发鼠标事件。
          symbol: 'circle',
          // 标记的图形（折线图交叉点）
          symbolSize: device ? 3 : 6,
          // 标记大小
    ...
    // 初始化饼状图
    pieChartInit: function pieChartInit(data) {
      // 初始化echarts实例
      var pieChart = echarts.init(document.getElementById('pieChart')); // 图表配置参数

      var option = {
        color: ['#19BCDD', '#33C4E1', '#4CCBE4', '#66D3E8', '#7FDAEC', '#7FDAEC', '#B2E9F4'],
    ...
    // 初始化环状图
    ringlikeChartInit: function ringlikeChartInit(platformCountyCnt, userCountyCnt, userCountyRate) {
      // 初始化echarts实例
      var ringlikeChart = echarts.init(document.getElementById('ringlikeChart')); // 图表配置参数
      ...
      var option = {
        ...
        series: [{
          ...
          labelLine: {
            show: true,
            length: device ? 8 : 10,
            length2: device ? 8 : 10 // lineStyle: {
            //     width: 2
            // }

          },
          data: [{
            value: platformCountyCnt,
            name: '平台服务区县'
          }, {
            value: userCountyCnt,
            name: '您的客户覆盖区县'
