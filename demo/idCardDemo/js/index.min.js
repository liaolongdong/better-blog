...
(function(window2, document2) {
  ...
  const searchBtn = document2.getElementById("searchBtn");
  searchBtn.addEventListener("click", function() {
    console.log(document2.getElementById("idCard"));
    const idCard = document2.getElementById("idCard").value;
    const resultNode = document2.getElementById("result");
    if (Validator.isValid(idCard)) {
      const idCardInfo = Validator.getInfo(idCard);
      console.log(idCardInfo);
      let resultHtml = "<p>户口所在地：{{addr}}</p><p>出生年月日：{{birth}}</p><p>性别：{{sex}}</p>";
